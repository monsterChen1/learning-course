      ...
      <div class="form">
        ...
        <div class="form-item">
          ...
          <div class="input">
            ...
            <button class="submit" @click="randomScore">随机改变</button>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      // list: localStorage.getItem('scoreList') ?
  ...
  watch: {
    // 简单写法
    // list(newVal, oldVal) {
    //   console.log('list数据发生变化了');
    //   localStorage.setItem('scoreList', JSON.stringify(newVal));
    // },
    list: {
      immediate: true,
      deep: true,
      handler(newVal, oldVal) {
        if(oldVal === undefined){
          this.list = localStorage.getItem('scoreList') ?
          JSON.parse(localStorage.getItem('scoreList')) : [];
        }else{
          localStorage.setItem('scoreList', JSON.stringify(this.list));
        }
      }
    }
  }
